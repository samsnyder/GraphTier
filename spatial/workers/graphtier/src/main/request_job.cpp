...
namespace graphtier {
  ...
  namespace request {
    ...
    RequestJob::RequestJob(RequestManager& requestManager,
      ...
      makeEntityRequest(fromEntityId, NodeData::ComponentId,
                        RequestCallback::RequestCallbackType::MyAttachedNetworks);

      makeEntityRequest(targetEntityId, NodeData::ComponentId,
    ...
    void RequestJob::requestNetworks(){
      ...
      while(!networksToRequest.empty()){
        ...
        this->reachableMtx.unlock();
        makeEntityRequest(networkId, NetworkData::ComponentId,
